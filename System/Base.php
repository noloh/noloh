 ...
 * Base class
 * 
 * A Base is the top-most parent of any NOLOH class. Its purpose is two-fold: First of all, for organizing classes, it is both convenient and
...
abstract class Base
{
	...
	function __set($nm, $val)
	{
		...
		elseif (strpos($nm, 'All') === 0 && $this instanceof Iterator)
		{
			...
			foreach($this as $obj)
			{
				if (is_object($obj) && (property_exists($obj, $prop) || ($obj instanceof Base && method_exists($obj, $method)) || ($obj instanceof Control && strpos($prop, 'CSS') === 0)))
				{
					$obj->$prop = $val;
				}
			}
