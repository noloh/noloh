...
//			$value = mysql_real_escape_string($value, $resource);
	...
	function DBDump($file, $compressionLevel = 5)
	{
		...
		if (PHP_OS === 'Linux')
		{
			...
			$backup = "PGPASSWORD={$pass} {$path} -F -h {$host} -U {$user} {$dbName}";
			...
			{
				$backup = "PGPASSWORD={$pass} {$path} -F -h {$host} -U {$user} -f {$file} {$dbName}";
				$compress = true;
			}
		}
		else
		{
			if ($this->Type === Data::Postgres)
			{
				$backup = "SET PGPASSWORD={$pass}&& pg_dump -F -h {$host} -U {$user} -d {$dbName}";
			}
			elseif ($this->Type === Data::MSSQL)
			{
